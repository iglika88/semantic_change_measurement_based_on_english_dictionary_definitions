...
* Brysbaert et al. supplies English concreteness ratings. Concreteness is the
  ...
  setting; the same calculation is used for cognate and MT mappings.
...
def definitions_concreteness(
    ...
    """Calculate lemma concreteness from all rated definition tokens.

    This is the same policy used in the finalized Kaggle notebook for every
    mapping family and setting. Each selected English definition is tokenized,
    every token found in the Brysbaert lookup contributes one rating, and the
    lemma score is the mean of the pooled token ratings. Definitions are never
    averaged separately and cognate/MT mappings use the same calculation.
    """
...
def definitions_for_concreteness(
    ...
    """Return exactly the definitions used for a lemma-level score.

    All-POS settings use the flattened dictionary order. Selected-POS settings
    use only the POS categories shared by the mapped Biblical and Modern lemma.
    For ``all`` settings, definitions from all eligible POS categories are
    pooled. For ``first`` settings, exactly one definition is retained: the
    first eligible definition in the dictionary's insertion order.
    """
...
def run_setting(
    ...
    for mapping in mapping_rows:
        old_lemma = mapping["biblical_lemma"]
        new_lemma = mapping["modern_lemma"]
        pos = mapping["pos"]
        old_defs = select_definitions(
            definitions_for(old_lemma, pos, bib), setting.definitions
        )
        new_defs = select_definitions(
            definitions_for(new_lemma, pos, modern), setting.definitions
        )
        if not old_defs or not new_defs:
            continue

        # Pair-level values are retained for auditability. The final
        # concreteness test below uses one score per Biblical lemma, calculated
        # from the complete definition set selected by the setting.
        old_pair_c = definitions_concreteness(old_defs, conc_lookup)
        new_pair_c = definitions_concreteness(new_defs, conc_lookup)
        ...
        pair_rows.append({
            ...
            "biblical_lemma": old_lemma,
            "modern_lemma": new_lemma,
            "pos": pos,
            "semantic_distance": apd(old_defs, new_defs, embeddings),
            "frequency": frequency.get(old_lemma, np.nan),
            "biblical_polysemy": polysemy.get(old_lemma, np.nan),
            "old_concreteness": old_pair_c["mean"],
            "new_concreteness": new_pair_c["mean"],
            "old_concreteness_matched_tokens": old_pair_c["matched_tokens"],
            "old_concreteness_all_tokens": old_pair_c["all_tokens"],
            "old_concreteness_coverage": old_pair_c["coverage"],
            "new_concreteness_matched_tokens": new_pair_c["matched_tokens"],
            "new_concreteness_all_tokens": new_pair_c["all_tokens"],
            "new_concreteness_coverage": new_pair_c["coverage"],
            "biblical_definitions": " || ".join(old_defs),
            "modern_definitions": " || ".join(new_defs),
        })
    ...
    for old_lemma, group in pair_df.groupby("biblical_lemma", sort=True):
        new_lemma = str(group["modern_lemma"].iloc[0])
        positions = group["pos"].astype(str).tolist()

        old_conc_defs = definitions_for_concreteness(
            old_lemma, positions, setting.definitions, bib
        )
        new_conc_defs = definitions_for_concreteness(
            new_lemma, positions, setting.definitions, modern
        )
        old_c = definitions_concreteness(old_conc_defs, conc_lookup)
        new_c = definitions_concreteness(new_conc_defs, conc_lookup)

        lemma_rows.append({
            "biblical_lemma": old_lemma,
            "semantic_distance": float(group["semantic_distance"].mean()),
            "frequency": group["frequency"].iloc[0],
            "biblical_polysemy": group["biblical_polysemy"].iloc[0],
            "old_concreteness": old_c["mean"],
            "new_concreteness": new_c["mean"],
            "old_concreteness_matched_tokens": old_c["matched_tokens"],
            "old_concreteness_all_tokens": old_c["all_tokens"],
            "old_concreteness_coverage": old_c["coverage"],
            "new_concreteness_matched_tokens": new_c["matched_tokens"],
            "new_concreteness_all_tokens": new_c["all_tokens"],
            "new_concreteness_coverage": new_c["coverage"],
            "biblical_concreteness_definitions": " || ".join(old_conc_defs),
            "modern_concreteness_definitions": " || ".join(new_conc_defs),
            "modern_lemmas": " | ".join(sorted(set(group["modern_lemma"]))),
            "positions": " | ".join(sorted(set(positions))),
        })

    lemma_df = pd.DataFrame(lemma_rows)
    lemma_df["concreteness_change"] = (
        lemma_df["new_concreteness"] - lemma_df["old_concreteness"]
    )

    frho, fp, fn = safe_spearman(
        lemma_df.frequency, lemma_df.semantic_distance
    )
    prho, pp, pn = safe_spearman(
        lemma_df.biblical_polysemy, lemma_df.semantic_distance
    )
    stat, cp, cn = safe_wilcoxon(
        lemma_df.old_concreteness, lemma_df.new_concreteness
    )
    mean_change = float(lemma_df.concreteness_change.mean())
    median_change = float(lemma_df.concreteness_change.median())
    percent_more_abstract = float(100 * (lemma_df.concreteness_change < 0).mean())
    ...
    summary = {
        ...
        "model": model_name,
        "mapping_setting": setting.mapping,
        "shared_pairs": len(pair_df),
        "shared_lemmas": len(lemma_df),
        "frequency_n": fn,
        "frequency_rho": frho,
        "frequency_p": fp,
        "frequency_result": symbol(fp, frho < 0),
        "polysemy_n": pn,
        "polysemy_rho": prho,
        "polysemy_p": pp,
        "polysemy_result": symbol(pp, prho > 0),
        "concreteness_n": cn,
        "old_concreteness_mean": lemma_df.old_concreteness.mean(),
        "new_concreteness_mean": lemma_df.new_concreteness.mean(),
        "mean_concreteness_change": mean_change,
        "median_concreteness_change": median_change,
        "percent_more_abstract": percent_more_abstract,
        "old_concreteness_coverage_mean": lemma_df.old_concreteness_coverage.mean(),
        "new_concreteness_coverage_mean": lemma_df.new_concreteness_coverage.mean(),
        "concreteness_wilcoxon": stat,
        "concreteness_p": cp,
        "concreteness_result": symbol(cp, mean_change < 0),
    }

    pair_df.to_csv(
        output_dir / f"greek_setting_{setting.number}_pair_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lemma_df.to_csv(
        output_dir / f"greek_setting_{setting.number}_lemma_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    print(
        f"\nSETTING {setting.number}: {setting.name}"
        f"\nlemmas={len(lemma_df):,} frequency={frho:.4g}/{fp:.4g} "
        f"polysemy={prho:.4g}/{pp:.4g} concreteness p={cp:.4g}"
    )
    return summary
