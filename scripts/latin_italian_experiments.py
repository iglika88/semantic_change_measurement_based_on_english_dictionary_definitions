#!/usr/bin/env python3
"""Latin–Italian dictionary-based semantic-change experiments.

The script compares Latin and Italian English-language dictionary definitions
under sixteen settings:

  1–8   normalized exact-cognate mapping
  9–16  conservative Latin-to-Italian machine-translation mapping
...
Final data policy
-----------------
* Kaikki/English Wiktionary supplies Latin and Italian lemmas, POS and English
  definitions.
...
* Cognate settings use exact normalized lemma matches only.
* Selected-POS settings additionally require an identical major lexical POS.

Conservative MT policy
----------------------
* MT output must resolve as a complete one-word or multiword expression to an
  Italian Kaikki entry; words inside an unmatched phrase are never matched
  independently.
* Surface and optional spaCy-lemmatized expressions are checked.
* Obvious non-lexical targets are removed.
* An Italian target may receive at most five Latin source lemmas by default.
...
Kaikki Latin and Italian JSONL files may be supplied locally or downloaded.
...
Run cognate Settings 1 and 8::
    ...
    python latin_italian_experiments.py \
...
Run MT Settings 9–16 from a translation cache::
    ...
    python latin_italian_experiments.py \
      ...
      --mt-cache latin_italian_mt_translations.csv
...
LibreTranslate endpoint. spaCy plus ``it_core_news_sm`` is optional but
...
def eligible_lemma_pair(
    ...
    italian: Mapping[str, Mapping[str, Sequence[str]]],
    min_length: int,
    exclude_proper_nouns: bool,
) -> bool:
    """Apply the finalized length and proper-noun filters."""
    if len(latin_lemma) < min_length or len(italian_lemma) < min_length:
        return False
    if exclude_proper_nouns and (
        has_proper_noun(latin_lemma, latin)
        or has_proper_noun(italian_lemma, italian)
    ):
        return False
    return True
...
def build_cognate_mapping(
    ...
    """Build normalized exact-lemma mappings for cognate settings."""
    rows: list[dict[str, str]] = []
    for lemma in sorted(set(latin) & set(italian)):
        if not eligible_lemma_pair(
            lemma, lemma, latin, italian, min_length, exclude_proper_nouns
        ):
            continue
        if setting.pos == "all":
            rows.append(
            ...
            shared = set(latin[lemma]) & set(italian[lemma]) & SELECTED_POS
...
class DeepLTranslator:
    ...
    def translate_many(self, texts: Sequence[str], batch_size: int) -> list[str]:
        ...
        for start in range(0, len(texts), batch_size):
            ...
            # Omitting source_lang lets the service attempt automatic detection.
            results = self.client.translate_text(
                list(texts[start : start + batch_size]), target_lang="IT"
            )
...
def load_spacy_italian(optional: bool = True) -> Any | None:
    try:
        import spacy
    except ImportError:
        if optional:
        ...
        raise
    try:
        return spacy.load("it_core_news_sm", disable=["ner", "parser"])
    except OSError:
        if optional:
            return None
        raise OSError(
            "Install the Italian spaCy model: python -m spacy download it_core_news_sm"
        )
...
def conservative_mt_mapping(
    ...
    for row, cleaned, doc in zip(
        ...
        target = italian_data.expression_aliases.get(surface_expression, "")
        if not target and lemma_expression:
            target = italian_data.expression_aliases.get(lemma_expression, "")
        if not target:
            compact_surface = surface_expression.replace(" ", "")
            compact_lemma = lemma_expression.replace(" ", "")
            if compact_surface in italian_data.entries:
                target = compact_surface
            elif compact_lemma in italian_data.entries:
        ...
        target_pos = (
            set(italian_data.entries.get(target, {})) & SELECTED_POS if target else set()
        )
...
def run_setting(
    ...
    for mapping in mapping_rows:
        ...
        new_lemma = mapping["italian_lemma"]
        pos = mapping["pos"]
        old_all = definitions_for(old_lemma, pos, latin)
        new_all = definitions_for(new_lemma, pos, italian)
        ...
        pair_rows.append(
            {
                ...
                "italian_concreteness": new_pair_concreteness["mean"],
                ...
                "italian_concreteness_matched_tokens": new_pair_concreteness[
                    "matched_tokens"
                ],
                "italian_concreteness_all_tokens": new_pair_concreteness["all_tokens"],
                "italian_concreteness_coverage": new_pair_concreteness["coverage"],
                "latin_definitions": " || ".join(old_defs),
                "italian_definitions": " || ".join(new_defs),
            }
        )
    ...
    for old_lemma, group in pair_dataframe.groupby("latin_lemma", sort=True):
        new_lemma = str(group["italian_lemma"].iloc[0])
        ...
        new_concreteness_defs = definitions_for_concreteness(
            new_lemma, positions, setting.definitions, italian
        )
        ...
        lemma_rows.append(
            {
                ...
                "italian_concreteness": new_concreteness["mean"],
                ...
                "italian_concreteness_matched_tokens": new_concreteness[
                    "matched_tokens"
                ],
                "italian_concreteness_all_tokens": new_concreteness["all_tokens"],
                "italian_concreteness_coverage": new_concreteness["coverage"],
                "latin_concreteness_definitions": " || ".join(
                    old_concreteness_defs
                ),
                "italian_concreteness_definitions": " || ".join(
                    new_concreteness_defs
                ),
                "italian_lemmas": " | ".join(sorted(set(group.italian_lemma))),
    ...
    lemma_dataframe["concreteness_change"] = (
        lemma_dataframe.italian_concreteness - lemma_dataframe.latin_concreteness
    )
    ...
    concreteness_stat, concreteness_p, concreteness_n = safe_wilcoxon(
        lemma_dataframe.latin_concreteness, lemma_dataframe.italian_concreteness
    )
    ...
    summary = {
        ...
        "italian_concreteness_mean": lemma_dataframe.italian_concreteness.mean(),
        ...
        "italian_concreteness_coverage_mean": (
            lemma_dataframe.italian_concreteness_coverage.mean()
        ),
    ...
    pair_dataframe.to_csv(
        output_dir / f"latin_italian_setting_{setting.number}_pair_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lemma_dataframe.to_csv(
        output_dir / f"latin_italian_setting_{setting.number}_lemma_results.csv",
        index=False,
        encoding="utf-8-sig",
    )
    print(
...
def parser() -> argparse.ArgumentParser:
    argument_parser = argparse.ArgumentParser(
        description="Run Latin-to-Italian dictionary semantic-change experiments."
    )
    argument_parser.add_argument("--latin-file", type=Path)
    argument_parser.add_argument("--italian-file", type=Path)
    ...
    argument_parser.add_argument(
        "--output-dir", type=Path, default=Path("latin_italian_results")
    )
...
def main() -> None:
    ...
    italian_path = ensure_resource(
        args.italian_file,
        resources / "kaikki.org-dictionary-Italian.jsonl",
        ITALIAN_KAIKKI_URL,
        not args.no_download,
    )

    latin_data = load_kaikki(latin_path, "Latin")
    italian_data = load_kaikki(italian_path, "Italian")
    ...
    pd.DataFrame(audit_rows).to_csv(
        args.output_dir / "latin_italian_preparation_audit.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    if mt_needed:
        if args.mt_cache:
            ...
            translations.to_csv(
                args.output_dir / "latin_italian_mt_translations.csv",
                index=False,
                encoding="utf-8-sig",
            )
        ...
        mt_dataframe = conservative_mt_mapping(
            ...
            latin_data.entries,
            italian_data,
            load_spacy_italian(optional=True),
            args.max_target_fanout,
            args.minimum_lemma_length,
            not args.include_proper_nouns,
        )
        mt_dataframe.to_csv(
            args.output_dir / "latin_italian_mt_conservative_mapping.csv",
            index=False,
            encoding="utf-8-sig",
        )
    ...
    for number in selected:
            ...
            for row in mt_dataframe.itertuples(index=False):
                if setting.pos == "all":
                    mappings.append(
                        {
                            "latin_lemma": row.latin_lemma,
                            "italian_lemma": row.accepted_target_lemma,
                            "pos": "all",
                            "match_type": "MT",
                        }
                    )
        ...
        for mapping in mappings:
            ...
            texts += select_definitions(
                definitions_for(
                    mapping["italian_lemma"], mapping["pos"], italian_data.entries
                ),
                setting.definitions,
            )
    ...
    pd.DataFrame(summaries).sort_values("setting").to_csv(
        args.output_dir / "latin_italian_settings_summary.csv",
        index=False,
        encoding="utf-8-sig",
    )
